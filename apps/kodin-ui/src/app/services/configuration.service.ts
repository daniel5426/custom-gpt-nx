...
import { DBkeys } from '../models-api/db-keys';
...
export class ConfigurationService {
  ...
  public baseUrl = Helpers.baseUrl();
  public loginUrl = 'environment.;';
  ...
  private loadLocalChanges() {
    ...
    if (this.localStorage.exists(DBkeys.THEME_ID)) {
      ...
      this.themeManager.installTheme(
        this.themeManager.getThemeByID(this._themeId as number)
      );
    } else {
      this.resetTheme();
    }
    ...
    if (jsonValue) {
      ...
      if (importValue.showDashboardNotifications != null) {
        this.showDashboardNotifications =
          importValue.showDashboardNotifications;
      }
  ...
  private addKeyToUserConfigKeys(configKey: string) {
    const configKeys =
      this.localStorage.getDataObject<string[]>(DBkeys.USER_CONFIG_KEYS) ?? [];
  ...
  private clearUserConfigKeys() {
    const configKeys = this.localStorage.getDataObject<string[]>(
      DBkeys.USER_CONFIG_KEYS
    );
