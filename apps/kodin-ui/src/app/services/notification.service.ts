...
import { NotificationEndpoint } from '../clients-api/notifications-api/notification-endpoint.service';
...
export class NotificationService {
  ...
  getNotification(notificationId: number) {
    return this.notificationEndpoint
      .getNotificationEndpoint<Notification>(notificationId)
      .pipe(
  ...
  getNotifications(page: number, pageSize: number) {
    return this.notificationEndpoint
      .getNotificationsEndpoint<Notification[]>(page, pageSize)
      .pipe(
        map((response) => {
  ...
  getUnreadNotifications(userId?: string) {
    return this.notificationEndpoint
      .getUnreadNotificationsEndpoint<Notification[]>(userId)
      .pipe(map((response) => this.getNotificationsFromResponse(response)));
  }

  getNewNotifications() {
    return this.notificationEndpoint
      .getNewNotificationsEndpoint<Notification[]>(this.lastNotificationDate)
      .pipe(
  ...
  getNewNotificationsPeriodically() {
    return interval(10000).pipe(
      ...
      mergeMap(() => {
        return this.notificationEndpoint
          .getNewNotificationsEndpoint<Notification[]>(
            this.lastNotificationDate
          )
          .pipe(
            map((response) =>
              this.processNewNotificationsFromResponse(response)
            )
          );
      })
    );
  ...
  pinUnpinNotification(
    ...
    if (typeof notification === 'number') {
      return this.notificationEndpoint.getPinUnpinNotificationEndpoint(
        notification,
        isPinned
      );
    } else {
  ...
  readUnreadNotification(notificationIds: number[], isRead: boolean) {
    return this.notificationEndpoint.getReadUnreadNotificationEndpoint(
      notificationIds,
      isRead
    );
  }
  ...
  deleteNotification(
    ...
    if (typeof notification === 'number') {
      return this.notificationEndpoint
        .getDeleteNotificationEndpoint<Notification>(notification)
        .pipe(
          map((response) => {
            if (response) {
              this._newNotifications = this.newNotifications?.filter(
                (n) => n.id !== notification
              );
